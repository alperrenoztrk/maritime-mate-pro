...
function BookLeafPager({
  ...
  const measure = useCallback(() => {
    ...
    // Quantize the leaf grid to whole CSS pixels and pin it on the flow. The
    // browser lays columns out on fractional widths; deriving offsets from a
    // rounded clientWidth lets a sub-pixel error accumulate per column, which
    // eighty columns in shears the resting spread and the turning-leaf faces.
    // With integer width/gap pinned, column starts land exactly on our stride.
    flow.style.columnGap = "";
    let gap = Math.round(Number.parseFloat(window.getComputedStyle(flow).columnGap) || 0);
    if ((width - gap) % 2 !== 0) gap = Math.max(0, gap - 1);
    ...
    // max-width must be pinned too: the class max-width:100% would clamp the
    // pinned width back to the pager's fractional width and reintroduce drift.
    flow.style.width = `${width}px`;
    flow.style.maxWidth = `${width}px`;
  ...
  const buildFace = useCallback((host: HTMLElement | null, left: number) => {
    ...
    // The face host is half a spread wide; without an absolute max-width the
    // class max-width:100% would squeeze the clone into the host and re-flow
    // its columns at half width — the printed leaf must keep the full grid.
    clone.style.width = `${metricsRef.current.width}px`;
    clone.style.maxWidth = `${metricsRef.current.width}px`;
    clone.style.height = `${metricsRef.current.height}px`;
    clone.style.columnGap = `${metricsRef.current.gap}px`;
  ...
  const beginTurn = useCallback((direction: BookTurnDirection): boolean => {
    ...
    // Re-measure synchronously: a resize or rotation whose rAF-scheduled
    // measure has not flushed yet must never print stale offsets onto a leaf.
    measure();
    const flow = flowRef.current;
    const leaf = leafRef.current;
    ...
    applyTurnFrame(0);
    return true;
  }, [applyTurnFrame, buildFace, measure]);
