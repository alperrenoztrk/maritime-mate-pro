...
export const fluidMechanicsTopicContents: Record<string, FluidTopicContent> = {
  ...
  "velocity-profile": {
    ...
    content:
      "Kaymama koşulu (no-slip condition) gereği, boru duvarına temas eden akışkan tabakasının hızı sıfırdır. Duvardan merkeze doğru hız artar.\n\nLaminer akışta hız profili parabolik eğri biçimindedir. Maksimum hız boru merkezinde oluşur ve ortalama hızın tam iki katına eşittir: V_max = 2·V_ort. Bu bağıntı, Hagen–Poiseuille analizi ile türetilir.\n\nTürbülanslı akışta karışma etkisi nedeniyle hız profili daha dolgun ve düz bir yapıdadır. Merkezle duvar arasındaki fark laminer akışa göre çok daha azdır. Türbülanslı akışta V_max/V_ort oranı yaklaşık 1.15-1.25 arasındadır ve Reynolds sayısı arttıkça bu oran 1'e yaklaşır.\n\nSayısal örnek: 100 mm iç çaplı boruda laminer akışla su geçiyor ve ortalama hız 0.5 m/s ise merkezde V_max = 2 × 0.5 = 1.0 m/s'dir. Debi Q = A·V_ort = π/4 × 0.1² × 0.5 = 3.93 × 10⁻³ m³/s = 3.93 L/s olarak hesaplanır.\n\nHız profili bilgisi, pitot tüpü ölçümlerinde ve debi hesaplarında kritik öneme sahiptir. Pitot tüpü noktasal hızı ölçtüğünden, ölçüm konumunun profil üzerindeki yeri bilinmeden doğru debi hesaplanamaz.",
    formula: {
      name: "Laminer Hız Profili",
      expression: "V(r) = V_max · [1 − (r/R)²]",
      description: "r: Merkeze uzaklık, R: Boru yarıçapı. Maksimum hız merkezde oluşur.",
    },
    keyPoints: [
      "Laminer akışta V_max = 2·V_ort (parabolik profil).",
      "Türbülanslı akışta V_max ≈ 1.2·V_ort (dolgun profil).",
  ...
  "minor-losses": {
    ...
    content:
      "Lokal kayıplar h_l = K·(V²/2g) formülüyle hesaplanır. Burada K kayıp katsayısı, V referans hızdır. K değeri bağlantı elemanının geometrisine bağlıdır ve deneysel olarak belirlenir.\n\nTipik K değerleri: 90° standart dirsek K ≈ 0.9, 45° dirsek K ≈ 0.4, T-parçası (dal) K ≈ 1.8, ani daralma K ≈ 0.5, ani genişleme K = (1 − A₁/A₂)², sürgülü vana (tam açık) K ≈ 0.2, kelebek vana (tam açık) K ≈ 0.3, çekvalf K ≈ 2.0.\n\nAlternatif yöntem olarak her bağlantı elemanı eşdeğer boru uzunluğu (L_e) olarak ifade edilebilir: h_l = f·(L_e/D)·(V²/2g). Bu yaklaşım, toplam kayıbın tek bir Darcy–Weisbach hesabıyla bulunmasını sağlar.\n\nSayısal örnek: Bir boru hattında 4 adet 90° dirsek (K=0.9), 2 adet sürgülü vana (K=0.2) ve 1 adet çekvalf (K=2.0) bulunmaktadır. Akış hızı V = 2.5 m/s ise toplam lokal kayıp: ΣK = 4×0.9 + 2×0.2 + 2.0 = 6.0. h_l = 6.0 × (2.5²/19.62) = 6.0 × 0.318 = 1.91 m.\n\nKısa boru hatlarında lokal kayıplar toplam kaybın büyük bölümünü oluşturabilir. Uzun boru hatlarında ise sürekli kayıplar baskındır ve lokal kayıplar toplam sürtünme kaybının %5-10'u kadar ek yük olarak tahmin edilebilir.",
    formula: {
      name: "Lokal Kayıp",
      expression: "h_l = K · (V² / 2g)",
  ...
  "fire-main": {
    ...
    formula: {
      ...
      expression: "H_pompa = H_hidrant + H_statik + h_f(toplam)",
