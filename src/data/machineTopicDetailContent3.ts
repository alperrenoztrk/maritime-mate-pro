...
const content3: ContentMap = {
  ...
  "cooling-hvac": {
    ...
    "COP (Performans Katsayısı) hesabı": {
      ...
      sections: [
        { heading: "COP Hesabı", paragraphs: ["Soğutma COP'u, evaporatörde alınan ısının kompresör tarafından harcanan iş enerjisine oranıdır. Isı pompası COP'u ise kondenserde atılan ısının kompresör işine oranıdır."], formula: { expression: "COPcooling = Qevaporator / Wcompressor = (h₁ − h₄) / (h₂ − h₁)\nCOPheatpump = COPcooling + 1", variables: ["Qevap: Soğutma kapasitesi (kW)", "Wkomp: Kompresör gücü (kW)"] }, example: { problem: "Bir gemi provizyon soğutma sisteminde evaporatörde 50 kW ısı alınıyor ve kompresör 18 kW güç tüketiyor. COP değerini hesaplayınız.", steps: ["COP = Qevap / Wkomp = 50 / 18 = 2.78"], result: "COP = 2.78, which means for every 1 kW of electrical energy consumed, 2.78 kW of cooling effect is obtained." } }
      ],
  ...
  "ship-systems": {
    "Dümen kuvveti ve tork hesabı": {
      ...
      sections: [
        { heading: "Dümen Kuvveti", paragraphs: ["Dümen paletine etkiyen normal kuvvet Joessel formülü ile yaklaşık hesaplanır."], formula: { expression: "FN = K × A × V² × sin(α)", variables: ["FN: Normal kuvvet (N)", "K: Katsayı (≈ 580 N·s²/m⁴ tuzlu su için)", "A: Dümen palet alanı (m²)", "V: Gemi hızı (m/s)", "α: Dümen açısı (derece)"] }, example: { problem: "Dümen alanı 12 m², gemi hızı 15 knot ve dümen açısı 35° ise dümen kuvvetini hesaplayınız.", steps: ["V = 15 × 0.5144 = 7.72 m/s", "FN = 580 × 12 × 7.72² × sin(35°)", "FN = 580 × 12 × 59.6 × 0.574", "FN = 238 100 N ≈ 238 kN"], result: "The force acting on the rudder blade is approximately 238 kN." } },
        { heading: "Dümen Torku", paragraphs: ["Tork, kuvvetin dümen şaftı eksenine göre momentinden hesaplanır. Dengeli dümen oranı arttıkça gerekli tork azalır."], formula: { expression: "T = FN × d", variables: ["d: Kuvvet merkezi ile dümen ekseni arası mesafe (m)", "Dengeli dümen: d = Aön × cön − Aarka × carka formüyle azaltılır"] } }
      ],
    ...
    "Pervane verimi ve kavitasyon": {
      ...
      sections: [
        { heading: "Pervane Verimi", paragraphs: ["Açık su verimi (η₀), gövde verimi (ηH) ve nispi dönme verimi (ηR) bileşenlerinden oluşan toplam pervane verimi genellikle %55-75 arasındadır."], formula: { expression: "ηD = η₀ × ηH × ηR", variables: ["η₀: Açık su verimi (pervane tek başına)", "ηH: Gövde verimi = (1-t)/(1-w)", "ηR: Nispi dönme verimi (≈ 1.0-1.05)", "t: İtme azaltma faktörü, w: iz faktörü"] } },
  ...
  "energy-efficiency": {
    ...
    "EEDI (Enerji Verimliliği Tasarım İndeksi)": {
      ...
      sections: [
        { heading: "EEDI Hesaplama Prensibi", paragraphs: ["EEDI, motorların yakıt tüketiminden kaynaklanan CO₂ emisyonunun taşıma kapasitesi ve referans hıza oranıdır."], formula: { expression: "EEDI = (Σ(CFj × SFCj × Pj) × fw) / (DWT × Vref)", variables: ["CFj: Yakıt tipi j karbon dönüşüm faktörü (g CO₂/g yakıt)", "SFCj: Özgül yakıt tüketimi (g/kWh)", "Pj: Motor gücü (kW)", "fw: Deniz koşulları düzeltme faktörü", "DWT: Yük kapasitesi (ton)", "Vref: Referans hız (knot)"] } },
