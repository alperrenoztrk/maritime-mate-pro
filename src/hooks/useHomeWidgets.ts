...
export function useHomeWidgets() {
  const [entries, setEntries] = useState<StoredEntry[]>(() => {
    ...
    // Seed defaults the first time. Every widget starts enabled so the home
    // screen looks exactly as it did while the toggles were missing.
    if (stored.length === 0) {
      return AVAILABLE_WIDGETS.map((w, i) => ({ id: w.id, enabled: true, order: i }));
    }
    // Merge in widget ids that were added after the user's choices were stored.
    const known = new Set(stored.map((e) => e.id));
    const merged = [...stored];
    AVAILABLE_WIDGETS.forEach((w, i) => {
      if (!known.has(w.id)) merged.push({ id: w.id, enabled: true, order: stored.length + i });
    });
    return merged;
  });
